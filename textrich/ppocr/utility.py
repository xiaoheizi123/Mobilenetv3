...
def draw_ocr_box_txt(
    ...
    for idx, (box, txt) in enumerate(zip(boxes, txts)):
        ...
        draw_left.polygon(box, outline=color, width=2)
...
def draw_in_one_canvas(
    ...
    draw_left = ImageDraw.Draw(img_plat)
    if txts is None or len(txts) != len(boxes):
        txts = [None] * len(boxes)
    for idx, (box, txt) in enumerate(zip(boxes, txts)):
        if scores is not None and scores[idx] < drop_score:
        ...
        box_w = int(max(np.linalg.norm(box[0] - box[1]), np.linalg.norm(box[2] - box[3])))
        box_h = int(max(np.linalg.norm(box[0] - box[3]), np.linalg.norm(box[1] - box[2])))
        if box_h/box_w >1.5:
            box[:, 0] += box_w+2
        else:
            box[:, 1] += box_h+2
        ...
        img_text_item = draw_box_txt_fine((w, h), box, txt, font_path)
        img_text = cv2.bitwise_and(img_text, img_text_item)

    # img_plat = cv2.bitwise_and(img_text, np.array(img_plat))
    img_plat.paste(Image.fromarray(img_text), (0, 0, w, h), mask=Image.fromarray(mask*125))

    return np.array(img_plat)
