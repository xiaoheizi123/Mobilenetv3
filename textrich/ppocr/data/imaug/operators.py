...
class DetResizeForTest(object):
    def __init__(self, **kwargs):
        ...
        if "image_shape" in kwargs:
            ...
            # if kwargs.get("keep_ratio_padding", False):
            #     self.resize_type = 3
    ...
    def resize_image_type1_pad(self, img):
        resize_h, resize_w = self.image_shape
        ori_h, ori_w = img.shape[:2]

        if resize_h > resize_w:
            ratio = float(resize_h) / resize_h
        else:
            ratio = float(resize_w) / resize_w
        ...
        im_pad = np.zeros((self.image_shape[0], self.image_shape[1], 3), np.uint8)
